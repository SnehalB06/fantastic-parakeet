...
const AdminTimesheets = () => {
  ...
  const handleApprove = async (ts) => {
    ...
    try {
      ...
      alert(translations[language].failedToApproveTimesheet || "Failed to approve timesheet");
    } finally {
      setProcessing(false);
    }
  };
  ...
  const handleReject = async (ts) => {
    setProcessing(true);
    try {
      await timesheetAPI.rejectTimesheet(getEmployeeIdString(ts), ts.month, ts.year);
      fetchTimesheets();
    } catch (err) {
      alert(translations[language].failedToRejectTimesheet || "Failed to reject timesheet");
    } finally {
      setProcessing(false);
    }
  };

  if (loading) return <div className="loading">{translations[language].loadingTimesheets || "Loading timesheets..."}</div>;
        ...
        <table className="timesheets-table">
          <thead>
            <tr>
              ...
              <th>{translations[language].actions || "Actions"}</th>
            </tr>
          </thead>
          <tbody>
            {filteredTimesheets.map(ts => (
              <tr key={`${ts._id}-${ts.month}-${ts.year}`}>
                ...
                <td>
                  {ts.status === "PENDING" && (
                    <>
                      <button className="btn btn-approve" onClick={() => handleApprove(ts)} disabled={processing}>{translations[language].approve || "Approve"}</button>
                      <button className="btn btn-reject" onClick={() => handleReject(ts)} disabled={processing}>{translations[language].reject || "Reject"}</button>
                    </>
                  )}
                </td>
              </tr>
